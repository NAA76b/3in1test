# -*- coding: utf-8 -*-
"""Master Workflow Tool (All-in-One) - Corrected

This version restores the email and document generation logic that was missing from the EmailerFrame class.
The four critical functions have been transplanted from the standalone script to fix the blank output bug.
"""
...
def log_message_util(root, log_widget, msg, level='info'):
    ...
    def _insert():
        ...
        log_widget.tag_config('success', foreground='#4CAF50', font=('Helvetica', 10, 'bold'))
        log_widget.tag_config('error', foreground='#F44336', font=('Helvetica', 10, 'bold'))
        log_widget.tag_config('info', foreground='#FFFFFF')
...
class ReporterFrame(ttk.Frame):
    def __init__(self, container, app_instance, **kwargs):
        ...
        ttk.Label(self, text="Pay Period Report Generator", style='Header.TLabel').grid(row=0, column=0, columnspan=3, sticky='w', pady=(0, 20))

        content_frame = ttk.Frame(self)
        content_frame.grid(row=1, column=0, sticky='nsew')
        content_frame.grid_columnconfigure(0, weight=1)

        ttk.Label(content_frame, text="1. Select History File").grid(row=0, column=0, sticky='w', pady=(0, 2))
        history_entry = ttk.Entry(content_frame, textvariable=self.history_file, state='disabled')
        history_entry.grid(row=1, column=0, sticky='ew', ipady=5)
        ttk.Button(content_frame, text="Browse...", command=lambda: self.browse_file(self.history_file, "Select History File")).grid(row=1, column=1, padx=10)

        ttk.Label(content_frame, text="2. Select Employee File").grid(row=2, column=0, sticky='w', pady=(15, 2))
        employee_entry = ttk.Entry(content_frame, textvariable=self.employee_file, state='disabled')
        employee_entry.grid(row=3, column=0, sticky='ew', ipady=5)
        ttk.Button(content_frame, text="Browse...", command=lambda: self.browse_file(self.employee_file, "Select Employee File")).grid(row=3, column=1, padx=10)

        ttk.Label(content_frame, text="3. Select Output Location").grid(row=4, column=0, sticky='w', pady=(15, 2))
        reporter_output_entry = ttk.Entry(content_frame, textvariable=self.reporter_output_location, state='disabled')
        reporter_output_entry.grid(row=5, column=0, sticky='ew', ipady=5)
        ttk.Button(content_frame, text="Browse...", command=lambda: self.browse_directory(self.reporter_output_location)).grid(row=5, column=1, padx=10)

        ttk.Label(content_frame, text="4. Select Fill Down Logic").grid(row=6, column=0, sticky='w', pady=(15, 2))
        ...
        fill_logic_combo.grid(row=7, column=0, sticky='ew', pady=(0, 15), ipady=5)

        action_frame = ttk.Frame(self)
        action_frame.grid(row=2, column=0, sticky='ew', pady=(20, 0))
        action_frame.grid_columnconfigure(0, weight=1)

        self.process_button = ttk.Button(action_frame, text="Generate Comparison Reports", command=self.run_process, style='Orange.TButton')
        self.process_button.grid(row=0, column=0, ipady=10, sticky='ew')

        self.progress = ttk.Progressbar(action_frame, orient='horizontal', mode='indeterminate', style='Orange.Horizontal.TProgressbar')
    ...
    def process_files_thread(self):
        try:
            ...
            unmatched_hist.to_excel(os.path.join(output_dir, f"Unmatched_History_{timestamp}.xlsx"), index=False)
            unmatched_emp.to_excel(os.path.join(output_dir, f"Unmatched_Employee_{timestamp}.xlsx"), index=False)
    ...
    def clean_report_data(self, filepath):
        ...
        find_and_map('Time Sheet Owner Name', [lambda c: 'time sheet' in c or 'timesheet' in c, lambda c: 'owner' in c, lambda c: 'name' in c])
        find_and_map('Compliance %', [lambda c: 'compliance' in c, lambda c: '%' in c])
        find_and_map('Super Office', [lambda c: 'super office' in c])
    ...
    def find_header_row(self, filepath):
        ...
        try:
            ...
            for i, row in df_preview.iterrows():
                ...
                if all(k in row_str for k in ['time sheet', 'compliance %', 'super office']):
...
class FormatterFrame(ttk.Frame):
    def __init__(self, container, app_instance, **kwargs):
        ...
        ttk.Label(self, text="Helper Sheet Data Formatter", style='Header.TLabel').pack(pady=(0, 20), anchor='w')

        inputs_frame = ttk.Frame(self)
        inputs_frame.pack(fill='x', pady=5)

        ttk.Label(inputs_frame, text="1. Select Source Excel File").grid(row=0, column=0, sticky='w', pady=(0, 2))
        file_entry = ttk.Entry(inputs_frame, textvariable=self.source_file, state='disabled')
        file_entry.grid(row=1, column=0, sticky='ew', ipady=5)
        ttk.Button(inputs_frame, text="Browse...", command=lambda: self.browse_file(self.source_file, "Select Source Excel File")).grid(row=1, column=1, padx=10)

        ttk.Label(inputs_frame, text="2. Select Pay Period").grid(row=2, column=0, sticky='w', pady=(15, 2))
        pp_combo = ttk.Combobox(inputs_frame, textvariable=self.pay_period, values=self.pp_display_list, state='readonly')
        pp_combo.grid(row=3, column=0, sticky='ew', ipady=5)
        if self.pp_display_list: pp_combo.set(self.pp_display_list[0])

        ttk.Label(inputs_frame, text="3. Select Report Day").grid(row=4, column=0, sticky='w', pady=(15, 2))
        day_combo = ttk.Combobox(inputs_frame, textvariable=self.report_day, values=["Monday", "Wednesday"], state='readonly')
        day_combo.grid(row=5, column=0, sticky='ew', ipady=5)

        ttk.Label(inputs_frame, text="4. Select Output Location").grid(row=6, column=0, sticky='w', pady=(15, 2))
        output_entry = ttk.Entry(inputs_frame, textvariable=self.output_location, state='disabled')
        output_entry.grid(row=7, column=0, sticky='ew', ipady=5)
        ttk.Button(inputs_frame, text="Browse...", command=lambda: self.browse_directory(self.output_location)).grid(row=7, column=1, padx=10)

        inputs_frame.grid_columnconfigure(0, weight=1)

        action_frame = ttk.Frame(self)
        action_frame.pack(expand=True, fill='both', pady=(20, 0))

        self.process_button = ttk.Button(action_frame, text="Generate Formatted Data", command=self.run_process, style='Blue.TButton')
        self.process_button.pack(fill='x', ipady=10)

        self.progress = ttk.Progressbar(action_frame, orient='horizontal', mode='indeterminate', style='Blue.Horizontal.TProgressbar')
...
class EmailerFrame(ttk.Frame):
    def __init__(self, container, app_instance, **kwargs):
        ...
        self.controls_frame.grid(row=1, column=0, sticky='nsw', padx=(0, 20))
        self.controls_frame.grid_rowconfigure(3, weight=1)

        ttk.Label(self.controls_frame, text="1. Select Data Digest File").pack(anchor='w')
        self.file_entry = ttk.Entry(self.controls_frame, textvariable=self.source_file, state='disabled')
        self.file_entry.pack(fill='x', ipady=5, pady=(2,0))
        ttk.Button(self.controls_frame, text="Browse...", command=self.load_data).pack(fill='x', pady=(5, 15))

        ttk.Label(self.controls_frame, text="2. Pay Period Being Reported").pack(anchor='w')
        self.pp_entry = ttk.Entry(self.controls_frame, textvariable=self.current_pp)
        self.pp_entry.pack(fill='x', ipady=5, pady=(2, 15))

        office_frame = ttk.LabelFrame(self.controls_frame, text="3. Select offices")
        office_frame.pack(fill='both', expand=True, pady=(0, 10))

        toggle_button_frame = ttk.Frame(office_frame)
        toggle_button_frame.pack(fill='x', pady=5, padx=5)
        ttk.Button(toggle_button_frame, text="Select All", command=lambda: self.toggle_all_offices(True)).pack(side='left', expand=True, padx=(0,5))
        ttk.Button(toggle_button_frame, text="Select None", command=lambda: self.toggle_all_offices(False)).pack(side='left', expand=True, padx=(5,0))

        self.office_canvas = tk.Canvas(office_frame, bg=self.app.WIDGET_BG, highlightthickness=0)
        self.office_list_frame = ttk.Frame(self.office_canvas)
        self.office_scrollbar = ttk.Scrollbar(office_frame, orient="vertical", command=self.office_canvas.yview)
        ...
        self.output_frame = ttk.LabelFrame(bottom_controls_frame, text="4. Select Output Format")
        self.output_frame.pack(fill='x', pady=(10, 10))
        ttk.Radiobutton(self.output_frame, text="Open in Outlook", variable=self.output_format, value="Outlook", command=self.toggle_word_path_visibility).pack(anchor='w', padx=10, pady=5)
        ttk.Radiobutton(self.output_frame, text="Generate Word Doc", variable=self.output_format, value="Word", command=self.toggle_word_path_visibility).pack(anchor='w', padx=10, pady=5)

        self.word_output_frame = ttk.LabelFrame(bottom_controls_frame, text="5. Word Document Filename (Optional)")
        self.word_output_entry = ttk.Entry(self.word_output_frame, textvariable=self.manual_filename)
        self.word_output_entry.pack(fill='x', expand=True, ipady=5, pady=5, padx=5)
        ttk.Label(self.word_output_frame, text="Format: YYYY.MM.DD_PP## Emails..._HHMMSS.docx", style='Info.TLabel').pack(padx=5, pady=(0,5), anchor='w')

        self.generate_button = ttk.Button(bottom_controls_frame, text="Generate Emails", style='Teal.TButton', command=self.run_generation, state='disabled')
        self.generate_button.pack(fill='x', ipady=10, pady=10)

        log_frame = ttk.LabelFrame(self, text="Emailer Log", padding="10")
        log_frame.grid(row=1, column=1, sticky='nsew')
        self.log_text = tk.Text(log_frame, state='disabled', bg=self.app.WIDGET_BG, fg=self.app.FG_COLOR, wrap='word', borderwidth=0)
        self.log_text.pack(expand=True, fill='both')

        self.toggle_word_path_visibility()

    def browse_word_location(self):
        directory = filedialog.askdirectory(initialdir=self.word_output_location.get(), title="Select Folder")
        if directory:
            self.word_output_location.set(directory)
        ...
        try:
            ...
            try:
                ...
                for sheet in xls.sheet_names:
                    try:
                        # Structural fingerprinting: Check cell H36 for "Super Office"
                ...
                # Rename columns based on expected names, handling variations
            ...
            for office_abbr in offices:
                ...
                cb = ttk.Checkbutton(self.office_list_frame, text=display_text, variable=var, style='TRadiobutton')
    ...
    def format_as_percent(self, value):
        log_message_util(self.app.root, self.log_text, f"VERBOSE: format_as_percent received value: '{value}' (type: {type(value)})")
        if pd.isna(value):
            log_message_util(self.app.root, self.log_text, "VERBOSE: Value is NA, returning empty string.")
            return ""
        try:
            if isinstance(value, str) and '%' in value:
                log_message_util(self.app.root, self.log_text, "VERBOSE: Value is already a percent string.")
                return value
            numeric_value = float(value)
            formatted_value = f"{numeric_value:.2%}"
            log_message_util(self.app.root, self.log_text, f"VERBOSE: Successfully formatted to '{formatted_value}'.")
            return formatted_value
        except (ValueError, TypeError):
            safe_value = str(value)
            log_message_util(self.app.root, self.log_text, f"VERBOSE: Could not format as percent. Returning safe string: '{safe_value}'.", 'error')
            return safe_value

    def get_next_pp_reminder(self, current_pp_num):
        log_message_util(self.app.root, self.log_text, f"VERBOSE: Searching for next PP reminder for current PP {current_pp_num}.")
        if self.dates_df is None:
            log_message_util(self.app.root, self.log_text, "VERBOSE: 'Dates' sheet not loaded. Returning default reminder.", 'error')
        ...
        try:
            ...
            log_message_util(self.app.root, self.log_text, f"VERBOSE: Next PP number is {next_pp_num}.")

            next_pp_data = self.dates_df[self.dates_df['Pay Period'] == next_pp_num].iloc[0]
            log_message_util(self.app.root, self.log_text, f"VERBOSE: Found data for PP {next_pp_num}.")

            start_date_raw = next_pp_data['PP Start Date']
            end_date_raw = next_pp_data['PP End Date']
            log_message_util(self.app.root, self.log_text, f"VERBOSE: Raw start date: {start_date_raw}, Raw end date: {end_date_raw}")

            # FIX: Use Windows-compatible date formatting to prevent crashes.
            start_date = pd.to_datetime(start_date_raw).strftime('%#m/%#d/%y')
            end_date = pd.to_datetime(end_date_raw).strftime('%#m/%#d/%y')
            log_message_util(self.app.root, self.log_text, f"VERBOSE: Formatted start date: {start_date}, Formatted end date: {end_date}")
            ...
            log_message_util(self.app.root, self.log_text, f"VERBOSE: Extracted deadline text: '{due_by_text}'")
            ...
            log_message_util(self.app.root, self.log_text, f"VERBOSE: Error in get_next_pp_reminder: {e}\n{traceback.format_exc()}", 'error')
            return "As a reminder, the ITR reporting deadline is 11:59 pm on Monday following the end of each Pay Period."

    # =========================================================================
    # START OF FIXED CODE: The following functions have been restored.
    # =========================================================================
    ...
    def create_html_body(self, data, director_info, current_pp_num, non_reporters_list):
        ...
        # This HTML structure is identical to the one in the standalone script.
...
class MasterWorkflowApp:
    def __init__(self):
        ...
        self.style.configure('Info.TLabel', background=self.BG_COLOR, foreground='#CCCCCC', font=('Helvetica', 9))
        self.style.configure('Header.TLabel', font=('Helvetica', 18, 'bold'))
        self.style.configure('TButton', background=self.WIDGET_BG, foreground=self.FG_COLOR, font=('Helvetica', 11), borderwidth=1)
        self.style.map('TButton', background=[('active', '#4a4a4a')])

        self.style.configure('Orange.TButton', background=self.ACCENT_COLOR_ORANGE, foreground=self.FG_COLOR, font=('Helvetica', 14, 'bold'))
        self.style.map('Orange.TButton', background=[('active', '#FF8533')])
        self.style.configure('Blue.TButton', background=self.ACCENT_COLOR_BLUE, foreground=self.FG_COLOR, font=('Helvetica', 14, 'bold'))
        self.style.map('Blue.TButton', background=[('active', '#0099FF')])
        self.style.configure('Teal.TButton', background=self.ACCENT_COLOR_TEAL, foreground=self.FG_COLOR, font=('Helvetica', 14, 'bold'))
        self.style.map('Teal.TButton', background=[('active', '#34c2b2')])

        self.style.configure('TCombobox', fieldbackground=self.WIDGET_BG, background=self.WIDGET_BG, foreground=self.FG_COLOR, arrowcolor=self.FG_COLOR)
        self.style.configure('TEntry', fieldbackground=self.WIDGET_BG, foreground=self.FG_COLOR, borderwidth=1)
        self.style.configure('Orange.Horizontal.TProgressbar', troughcolor=self.WIDGET_BG, background=self.ACCENT_COLOR_ORANGE, thickness=20)
        self.style.configure('Blue.Horizontal.TProgressbar', troughcolor=self.WIDGET_BG, background=self.ACCENT_COLOR_BLUE, thickness=20)
        self.style.configure('Teal.Horizontal.TProgressbar', troughcolor=self.WIDGET_BG, background=self.ACCENT_COLOR_TEAL, thickness=20)
        ...
        self.style.configure('TNotebook.Tab', background=self.WIDGET_BG, foreground=self.FG_COLOR, padding=[10, 5], font=('Helvetica', 11, 'bold'))
        self.style.map('TNotebook.Tab', background=[('selected', self.ACCENT_COLOR_BLUE)], foreground=[('selected', self.FG_COLOR)])
        self.style.configure('TRadiobutton', background=self.BG_COLOR, foreground=self.FG_COLOR, font=('Helvetica', 11))
        self.style.map('TRadiobutton', indicatorcolor=[('selected', self.ACCENT_COLOR_TEAL)], background=[('active', self.BG_COLOR)])
